...
页面展示的价格单位读取自当前清洗结果。V2.0 标准输出为 `元/kg`：原始 `元/斤` 会乘以 2，原始 `元/kg` 保持不变；缺少映射菜价的植物显示“未映射菜价”，不会偷偷补一个 0 元白菜价。
...
默认 150 阳光、20 格的结果中，小喷菇数量较多是当前模型的真实输出：它的真实阳光成本为 0、单株相对评分为正，同时优化器只有总阳光、总格子和最低阵容角色约束，没有植物多样性或单品数量上限。土豆雷和灯笼草满足高分输出及防御/控制要求后，剩余格子继续放入不增加阳光消耗的小喷菇，因此默认组合会出现小喷菇 ×14。它并没有“无限免费”：每株仍占 1 个格子，评分分母也使用 25 点保护值。本项目保留这一结果，不为了画面好看偷偷拔蘑菇。
...
“模型说明与技术细节”默认折叠，内部按标签页提供评分公式与权重、完整植物排名、优化约束与数据映射、必要调试状态。草坪格子属于**阵容展示布局**，只核对数量和容量，不是优化器输出的最优坐标。
...
- V2.0 优化器不包含植物多样性、轮作、土地质量、生长周期和单品数量上限，因此零阳光植物可能大量占用剩余格子。
...
- 浏览器控制台偶尔出现 Vega `Infinite extent` 或 scale binding 警告：这是 Streamlit 原生图表在初始化交互范围时短暂使用空选择域导致的渲染周期提示。页面已在绘图前拦截空数据、单条数据和恒定序列；实际图表、数值和推荐不受影响，因此 V2.0 不为隐藏控制台提示而更换图表系统或增加依赖。
