...
PvZ Economy Engine V2.1.0 是一个“看起来在打僵尸，实际上认真做数据分析”的蔬菜种植推荐系统。它把北京示例数据或用户上传的任意地区菜价、植物策略属性和有限资源约束放进同一块草坪里，专门研究一个非常严肃、也非常离谱的问题：**如果末日真的来了，有限的阳光和格子到底该种什么，才能既守住脑子，又守住钱包？**
...
页面展示的价格单位读取自当前清洗结果。V2.1.0 标准输出为 `元/kg`：原始 `元/斤` 会乘以 2，原始 `元/kg` 保持不变；缺少映射菜价的植物显示“未映射菜价”，不会偷偷补一个 0 元白菜价。
...
- V2.1.0 使用固定 30% 草坪占比作为单植物集中度上限；该比例是可解释建模假设，不是现实试验得出的最佳值，也不等同于强制阵容多样性。
- V2.1.0 优化器仍不包含轮作、土地质量、生长周期和重复种植边际收益递减。
...
- 浏览器控制台偶尔出现 Vega `Infinite extent` 或 scale binding 警告：这是 Streamlit 原生图表在初始化交互范围时短暂使用空选择域导致的渲染周期提示。页面已在绘图前拦截空数据、单条数据和恒定序列；实际图表、数值和推荐不受影响，因此 V2.1.0 不为隐藏控制台提示而更换图表系统或增加依赖。
